...
function KpiCard({
  ...
  return (
    <Card
      className="group overflow-hidden border border-card-border bg-card transition-all duration-300 hover:shadow-xl hover:-translate-y-1"
      style={{ borderRadius: "18px" }}
    >
      <CardContent className="relative p-6">

        {/* subtle background glow */}
        <div className="absolute right-0 top-0 h-24 w-24 rounded-full bg-primary/5 blur-3xl transition-all duration-300 group-hover:bg-primary/10" />
        ...
        <div className="mt-6 flex items-end gap-1">
          {[30, 45, 35, 60, 50, 70, 65, 78].map((h, i) => (
            <div
              key={i}
              className="flex-1 rounded-full bg-primary/70 transition-all duration-300 group-hover:bg-primary"
              style={{
                height: `${h / 2}px`,
                opacity: 0.2 + i * 0.08,
              }}
