...
import {
  ArrowLeft, Pencil, Copy, FileDown, ChevronRight,
...
import { useInvoice, useDuplicateInvoice, useGenerateInvoicePdf } from "@/hooks/use-invoices";
...
export default function ShipperInvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id }   = use(params);
  ...
  if (!invoice) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4">
        ...
        <Link href="/shipper/invoices" className="text-sm text-primary underline">Back to invoices</Link>
      </div>
    );
  }
  ...
  async function handleDuplicate() {
    try { await duplicateMut.mutateAsync(id); toast.success("Invoice duplicated"); router.push("/shipper/invoices"); }
        ...
        <div className="mx-auto max-w-6xl px-4 py-4 sm:px-6">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {isDraft && (
                <Button size="sm" onClick={() => router.push(`/shipper/invoices/${id}/edit`)}
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            ...
            <PricingSummary
              ...
              taxRate={invoice.tax_rate} tax={invoice.tax} total={invoice.total}
              amountPaid={invoice.amount_paid} balanceDue={invoice.balance_due} readOnly
            />
          </div>
