...
export async function updateStatus(
  ...
  if (dto.status === 'delivered' && shipment.customer_id) {
    void rewardsCreditService.earnCreditForDelivery(shipment.customer_id as string, id)
  }
...
export async function assignDriver(shipmentId: string, dto: AssignEmployeeDto) {
  ...
  if (dto.employeeId !== null) {
    const { data: driver, error: driverErr } = await supabase
      .from('profiles')
      .select('id, full_name, role, admin_role, is_active')
      .eq('id', dto.employeeId)
      .eq('role', 'admin')
      .eq('admin_role', 'driver')
      .single()

    if (driverErr || !driver) throw AppError.notFound('Driver')
    if (!driver.is_active) throw AppError.badRequest('This driver is not active')

    notifyUser(dto.employeeId, 'shipment_assigned', 'Delivery assigned to you', 'A delivery has been assigned to you.', shipmentId)
  }

  const { data, error } = await shipmentsRepo.updateById(shipmentId, {
    assigned_employee_id: dto.employeeId,
  })
  if (error || !data) throw AppError.internal('Failed to assign driver', error)
